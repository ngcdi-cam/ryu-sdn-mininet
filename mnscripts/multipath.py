...
class MULTIPATH_13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.logger.info("switch:%s connected \n", dpid)
        #self.logger.info("switch actions: %s \n", repr(actions))
    ...
    def mac_learning(self, dpid, src_mac, in_port):
        ...
        if src_mac in self.mac_to_port[dpid]:
            self.logger.info('Found Mac: %s -> %s\n',src_mac,str(self.mac_to_port[dpid]))
            ...
            self.logger.info("["+ str(dpid) + "] --> [" + src_mac + "] = " + str(in_port) + "\n")
            return True
    ...
    def send_group_mod(self, datapath,):
        ...
        self.logger.info("Group done: ")
        self.logger.info(req)
        datapath.send_msg(req)

    def print_macs_to_port(self):
            for k in self.mac_to_port.keys():
                for mac in  self.mac_to_port.get(k).keys():
                    self.logger.info("key: %s => mac: %s\n",k,mac)                        
    ...
    def port_delete_handler(self, ev):
        ...
        self.print_macs_to_port()
        self.logger.info('<====Before====>')
        
        if reason == ofproto.OFPPR_MODIFY:
            self.mac_to_port.clear()
            self.datapaths.clear()
            # self.logger.info("!!!!!!!!!!!!!START PORT REMOVED!!!!!!!!!!!!!!!!!\n")                    
            # macs_to_update = []
            # for k in self.mac_to_port.keys():
            #     if k == datapath.id:
            #         mac_to_port_table = self.mac_to_port.get(k)
            #         for mac in mac_to_port_table.keys():
            #             out_port = mac_to_port_table[mac]
            #             if(out_port == port_no):
            #                 #macs_to_update = macs_to_update + [mac]
            #                 if k == 4:
            #                     mac_to_port_table[mac] = 1
            #                 elif k == 5:
            #                     mac_to_port_table[mac] = 2
                        
            # self.logger.info("List to update: \n")                    
            # for l in macs_to_update:
            #     self.logger.info(' mac to update: %s \n',l)
                
            # for k in self.mac_to_port.keys():                
            #     new_mac_to_port_table = {}
            #     mac_to_port_table = self.mac_to_port.get(k)
            #     for mac in mac_to_port_table.keys():
            #         if mac not in macs_to_update:
            #             out_port = mac_to_port_table[mac]
            #             new_mac_to_port_table[mac] = out_port
                    
            #         for nmtpt in new_mac_to_port_table.keys():
            #             self.logger.info('key[%s] -> new_mac_to_port_table [%s] = %s\n',k,nmtpt,str(new_mac_to_port_table.get(nmtpt)))
                    
            #     self.mac_to_port[k] = new_mac_to_port_table
            ...
            self.logger.info('Port_no: '+str(port_no)+'\n')
            self.logger.info('reason: '+str(reason)+'\n')
            self.logger.info('dp: '+repr(ev.msg.datapath.id)+'\n')
            self.logger.info("\n!!!!!!!!!!!!! END PORT REMOVED!!!!!!!!!!!!!!!!!\n")
        else:
            self.logger.info("Illegal port state %s %s", port_no, reason)
        #self.logger.info('==PORT DELETED==>'+str(dp)+'==>'+str(port)+"<==\n")
        

    #Main Packet Handler/Dispatcher
    @set_ev_cls(ofp_event.EventOFPPacketIn, MAIN_DISPATCHER)
    def _packet_in_handler(self, ev):
        msg = ev.msg
        datapath = msg.datapath
        #self.logger.info("the dp==>"+repr(datapath)+"<==\n")
        dpid = datapath.id
        parser = datapath.ofproto_parser
        in_port = msg.match['in_port']
        ...
        # for t in self.datapaths:
        #     self.logger.info('dp->'+repr(t)+'\n')
        #-------------#
        
        
        if isinstance(ip_pkt_6, ipv6.ipv6):
            self.logger.debug("[%s] IPv6 processing: %s -> %s \n",dpid,ip_pkt_6.src,ip_pkt_6.dst)
            actions = []
        ...
        if isinstance(arp_pkt, arp.arp):
            self.logger.debug("[%s] ARP processing: %s (%s) -> %s OPCODE: %s \n",dpid,arp_pkt.src_mac,in_port,arp_pkt.dst_mac,arp_pkt.opcode)
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            self.logger.info("[%s] IPV4 processing: %s -> %s \n",dpid,ip_pkt.src,ip_pkt.dst)
            #self.logger.info("the packet==>"+repr(ip_pkt))
            mac_to_port_table = self.mac_to_port.get(dpid)
            # ip_to_port_table = self.ip_to_port.get(dpid)
            if mac_to_port_table is None:
                #self.logger.info(dpid)
                self.logger.debug("Dpid is not in mac_to_port")
                return
            
            # for x in mac_to_port_table:
            #     self.logger.info("===>"+x+"\n")
            ...
            if eth.dst in mac_to_port_table:
                ...
                if ip_pkt != None:
                    ipsrc = ip_pkt.src
                if self.mac_learning(dpid, eth.src, ipsrc, in_port) is False:
